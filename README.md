...
i.	 You can press “1” to encrypt a new message.
ii.	 Press “2” to decrypt an existing message.
iii. Press “3” to exit from the application.
iv.	 Press “4” to view the Main Dashboard and Help Text once again.
...
•	Although this application can be used to encrypt small messages, this level of encryption is NOT SECURE for sensitive data since Encrypt0 only uses a few levels of Classical Era ciphers for encryption.
•	Since Encrypt0 is based on Classical Era Ciphers, it does not add any functions to the fundamental working of those ciphers. Therefore, Encrypt0 can only encrypt alphabetical messages.
•	The file usage structure is rigid in Encrypt0. If not careful, previously encrypted files can get overwritten by the application (as warned earlier).
•	Finally, “KeyboardInterrupts” have not been handled to the fullest extent.
...
•	The source code of the whole application is provided in the “source” folder. 
•	The Python Library named “PyInstaller” has been used to create the standalone frozen binary executable for Encrypt0, which is implemented in the C Programming Language.
